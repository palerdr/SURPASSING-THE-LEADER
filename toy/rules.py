"""Standalone rules for the single canonical enumerated toy."""
...
def ruleset_for_name(name: str, *, max_half_rounds: int = 8) -> ToyRuleset:
    if name != "bucket12_fixed50":
        raise ValueError(f"unknown toy ruleset {name!r}; expected 'bucket12_fixed50'")
    return Bucket12Fixed50Rules(max_half_rounds=max_half_rounds)

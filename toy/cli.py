...
from toy.artifacts import canonical_json
...
from toy.rules import ruleset_for_name
from toy.self_play import ToySelfPlayConfig, generate_self_play, write_self_play
from toy.tablebase import build_tablebase, load_tablebase, write_tablebase
from toy.targets import build_exact_targets, write_exact_targets
...
def _output_dir(args: argparse.Namespace, ruleset_id: str) -> Path:
    return Path(args.output_dir) if args.output_dir else Path("toy") / "outputs" / ruleset_id
...
def command_train(args: argparse.Namespace) -> int:
    rules = _rules(args)
    output_dir = Path(args.output_dir) if args.output_dir else Path("toy") / "outputs" / rules.ruleset_id / "checkpoint"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="python -m toy.cli")

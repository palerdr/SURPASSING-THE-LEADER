...
`src/dth/solver.py` is the complete mathematical implementation of the first
...
`src/dth/generate_dataset.py` owns reachable-state enumeration and NPZ writing.
`src/dth/config/dataset.yaml` is its Hydra configuration. Dataset concerns must
not move back into the solver.

`src/dth/docs/GAME_AND_SOLVER.md` is the mathematical specification. Code and documentation must
...
dependencies to `src/dth/solver.py`.
...
The default parameters live in `src/dth/config/dataset.yaml`. Any field can be
overridden without editing code, for example:

```powershell
uv run python -m dth.generate_dataset horizon=2 output=src/dth/artifacts/exact_h2.npz
uv run python -m dth.generate_dataset horizon=1 root_states='[[239,0,0,0],[240,0,0,0]]' output=src/dth/artifacts/boundary_h1.npz
```
...
The default parameters live in `src/dth/config/train.yaml`. Training uses a
...
`src/dth/checkpoints/strategic_exact_v1/`.

The richer horizon-3 follow-up is reproducible with:

```powershell
uv run python -m dth.generate_dataset --config-name strategic_exact_v2
uv run python -m dth.train dataset=src/dth/artifacts/strategic_exact_v2.npz output_dir=src/dth/checkpoints/strategic_exact_v2
```
...
```text
src/dth/artifacts/exact_h3.npz
```
...
`src/dth/torch_cfr.py` unrolls a fixed, differentiable CFR+ budget over the three
...
`src/dth/checkpoints/strategic_matrix_policy_cfr_v13/` selected epoch 200.  With 64
...
`src/dth/artifacts/readiness_development_v2.npz`.

# Pure Drop the Handkerchief state and exact finite-horizon solver

`src/dth/solver.py` is the executable authority for this abstraction. This
...
Hydra loads `src/dth/config/dataset.yaml`; `horizon`, `output`, `root_states`, and
`progress_every` are command-line overridable. `src/dth/solver.py` performs no
...
| `schema_version` | scalar | string | `dth-v1-ttd-strict-overflow` |

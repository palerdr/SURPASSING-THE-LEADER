...
def solve_minimax(payoff: np.ndarray) -> tuple[np.ndarray, float]:
    ...
    result = linprog(
        ...
        method="highs",
        # HiGHS otherwise permits roughly 1e-7 primal/dual residuals.  That
        # is looser than the solver firewall's independently recomputed 1e-8
        # certificate and can make a valid search abort nondeterministically.
        # Ask the optimizer for a stricter solution; do not relax the
        # independent certificate below.
        options={
...
def solve_cfr_plus_rust(
    ...
    try:
        ...
        raise ImportError(
            ...
            "`src/crates/stl_solver` to enable the Rust CFR+ kernel"
        ) from exc

...
def _args(**overrides):
    defaults = dict(
        base_targets="src/stl/checkpoints/base_targets.npz",
        held_out_targets="src/stl/checkpoints/holdout.npz",
...
def test_next_run_excludes_d1_after_live_strength_rejection():
    accepted = {
        "status": "accepted",
        "checkpoint": "src/stl/checkpoints/current/best.pt",
        "held_out_overall_mse": 0.0559,
    }
    rejected = [
        {
            "checkpoint": "src/stl/checkpoints/gen_tier_a_d1hal/best.pt",
...
def test_next_run_switches_to_mcts_refresh_after_d0_append_rejection():
    ...
    rejected = [
        {
            "checkpoint": "src/stl/checkpoints/gen_tier_a_aux_plus_d0_150k/best.pt",
    ...
    assert "--init-checkpoint src/stl/checkpoints/current/best.pt" in next_run["training_command"]
...
def test_next_run_requires_pattern_reader_hardening_after_mcts_rejection():
    ...
    rejected = [
        {
            "checkpoint": "src/stl/checkpoints/gen_current_mcts300_tiera50k/best.pt",
            "reasons": ["ladder wins_delta -10 < required 1"],
            "ladder_wins_delta": -10,

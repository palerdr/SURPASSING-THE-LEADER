...
def test_development_shard_is_model_selection_only(tmp_path: Path):
    path = tmp_path / "development.npz"
    save_replay_shard([_record(21)], path, shard_role=ShardRole.DEVELOPMENT)

    assert len(load_replay_shard(path, expected_role=ShardRole.DEVELOPMENT)) == 1
    with pytest.raises(ReplayValidationError, match="forbidden in training"):
        load_replay_shard(path, for_training=True)
    with pytest.raises(ReplayValidationError, match="required role"):
        load_replay_shard(path, expected_role=ShardRole.EXTERNAL_RULER)
...
def test_tier_a_midpoint_requires_bounds_inactive_policy_and_zero_cutoff(
    tmp_path: Path,
):
    base = _record(42)
    ...
    for index, scenario in enumerate(pinned_scenarios()):
        ...
        records.append(
            TrainingRecordV3(
                ...
                checker_legal_mask=checker_mask,
                state_origin=StateOrigin.TACTICAL_TABLEBASE,
                source_artifact=f"src/stl/solver/tablebase.py::{scenario.name}",

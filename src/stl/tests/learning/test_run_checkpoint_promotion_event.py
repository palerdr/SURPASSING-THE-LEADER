...
def _calibration(*, passed=True, beats=True):
    return {
        "checkpoint": "src/stl/checkpoints/candidate/best.pt",
        "gate_passed": passed,
...
def _ladder(overall_delta: int, opponent_deltas=None):
    ...
    return {
        "config": {
            "champion_checkpoint": "src/stl/checkpoints/champion/best.pt",
            "candidate_checkpoint": "src/stl/checkpoints/candidate/best.pt",
            "agent_iterations": 30,
        },
        "aggregate": {
...
def test_decide_promotion_rejects_trace_policy_gate_checkpoint_mismatch():
    ...
    trace_gate["config"]["candidate_checkpoint"] = "src/stl/checkpoints/other/best.pt"
...
def test_decide_promotion_rejects_candidate_checkpoint_mismatch():
    ladder = _ladder(5)
    ladder["config"]["candidate_checkpoint"] = "src/stl/checkpoints/other/best.pt"
...
def test_decide_promotion_rejects_champion_checkpoint_mismatch():
    ...
    exploitability["config"]["champion_checkpoint"] = "src/stl/checkpoints/other_champion/best.pt"
...
def test_decide_promotion_accepts_relative_and_absolute_equivalent_paths():
    ...
    calibration["checkpoint"] = str(Path("src/stl/checkpoints/candidate/best.pt").resolve(strict=False))
...
def test_decide_promotion_rejects_policy_drift_checkpoint_mismatch():
    ...
    policy_drift["config"]["candidate_checkpoint"] = "src/stl/checkpoints/other/best.pt"

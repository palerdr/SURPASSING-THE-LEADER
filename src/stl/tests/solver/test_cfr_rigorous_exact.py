...
def test_minimax_requests_residuals_stricter_than_its_certificate(monkeypatch):
    import stl.solver.exact as exact_mod
    ...
    def fake_linprog(*args, **kwargs):
        ...
        return SimpleNamespace(
            ...
            status=0,
            message="forced exact solution",
        )

    monkeypatch.setattr(exact_mod, "linprog", fake_linprog)

    strategy, value = solve_minimax(
        np.array([[1.0, -1.0], [-1.0, 1.0]])
    )

    np.testing.assert_array_equal(strategy, [0.5, 0.5])
    assert value == 0.0
    options = observed["options"]
    assert isinstance(options, dict)
    assert options["primal_feasibility_tolerance"] < 1e-8
    assert options["dual_feasibility_tolerance"] < 1e-8
    assert options["ipm_optimality_tolerance"] <= 1e-10
...
def test_rigorous_cfr_modules_do_not_import_reward_or_value_heuristics():
    ...
    rigorous_files = [
        root / "src/stl/solver/exact.py",
        root / "src/stl/solver/search.py",
        root / "src/stl/solver/tablebase.py",
    ]
    forbidden = (
        "stl.learning.reward",
        "ROUTE_SHAPING",
        "shaped_reward",
        "stl.learning.evaluate",
        "value_net",

...
Usage (programmatic):
    ...
    bootstrap_one_generation(
        gen_in_checkpoint="src/stl/checkpoints/gen0/best.pt",
        gen_out_targets="targets/gen1.npz",
        gen_out_dir="src/stl/checkpoints/gen1",
...
from stl.learning.train import TrainConfig, TrainResult, load_checkpoint, make_predict_fn, train
...
def bootstrap_one_generation(
    *,
    ...
    if config.run_audit_pack:
        ...
        audit_evaluator = ValueNetEvaluator(
            model_fn=audit_predict, default_value_horizon=2
        )
...
if __name__ == "__main__":
    ...
    if args.held_out_targets is not None:
        from stl.learning.targets import load_targets_as_records

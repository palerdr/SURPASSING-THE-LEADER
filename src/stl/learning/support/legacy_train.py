...
def train_value_net(
    ...
    for epoch in range(epochs):
        ...
        for start in range(0, N, batch_size):
            ...
            predictions = value_output(net(batch_features, 2))
...
def load_checkpoint(path: str, n_features: int = FEATURE_DIM) -> ValueNet:
    ...
    if "trunk.0.weight" in state:
        ...
        if state["trunk.0.weight"].shape[1] == FEATURE_DIM:
            imported = net.state_dict()
            for key, value in state.items():
                if key == "trunk.0.weight":
                    imported[key].zero_()
                    imported[key][:, :FEATURE_DIM].copy_(value)
                elif key in imported and imported[key].shape == value.shape:
                    imported[key].copy_(value)
                else:
                    raise RuntimeError(
                        f"Horizon-blind checkpoint parameter {key!r} is incompatible"
                    )
            net.load_state_dict(imported, strict=True)
        else:
        ...
        net = ValueNet(n_features, hidden_dim=hidden_dim)
        old_first_layer = migrated["trunk.0.weight"]
        padded_first_layer = net.state_dict()["trunk.0.weight"].clone()
        padded_first_layer.zero_()
        padded_first_layer[:, :FEATURE_DIM].copy_(old_first_layer)
        migrated["trunk.0.weight"] = padded_first_layer

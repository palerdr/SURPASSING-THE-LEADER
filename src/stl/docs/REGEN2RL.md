...
- [x] **P3 MCTS improvement operator.** Canonical linearly weighted mean-Q
  ...
  `src/stl/outputs/regen2rl/p3_mcts_conformance.json`, canonical report-payload SHA-256
...
new work under `src/crates/` is part of this plan. A Rust kernel follows only after
...
- `src/stl/engine/README.md` for rule authority and actor-aware legality.
- `src/stl/solver/README.md` for exact-second minimax, terminal-only utility, and
  the solver firewall.
- `src/stl/learning/README.md` for the length-62 policy contract and the current
  target/model/training boundary.
- `src/stl/play/README.md` for the quarantine boundary around bucketed playable
  code.
...
|---|---|---|
| Engine authority | `src/stl/engine/game.py`, `src/stl/engine/actions.py`, engine tests | Keep unchanged except for discovered rule bugs. |
| Exact local game | `src/stl/solver/exact.py` enumerates literal seconds, expands engine chance, and solves LP minimax | Certification oracle for bounded horizons. |
| Search | `src/stl/solver/search.py`, `conformance.py`, and `mcts_conformance.py` | P1/P3 conformance complete; candidate mode remains approximate and audited. |
| Exact anchors | `src/stl/solver/tablebase.py` and `src/stl/learning/targets.py` | Retain as immutable replay strata. |
| Policy/value model | `src/stl/learning/model.py` emits one Hal value and two length-62 role heads from 52 named V2 features | P2 schema complete; empirical fit remains a P4 gate. |
| Replay/checkpoints | `src/stl/learning/replay.py` and `train.py` provide reconstructable V3 shards, grouped splits, and strict V2 checkpoint bundles | P2 integrity boundary complete; rejected V2 Gen-0 shards are not migrated. |
| Supervised learner | `src/stl/learning/train.py` has masked role-policy losses, weighted value loss, grouped selection, and V3 replay loading | The first corrected full candidate trained successfully but failed its external ruler. |
| Evaluation pieces | `src/stl/commands/gen0_eval.py` now provides the P4 external-ruler gate; calibration, audit, ladder, best-response intervals, SPRT, and promotion modules remain separate | Integrate the post-P4 pieces into one mandatory promotion gate with precise claim labels. |

### Why the current tree is not yet full AlphaZero reinforcement learning

1. The audit found that the former `src/stl/config/command/self_play.yaml` dispatched
...
2. `src/stl/learning/support/self_play.py` runs bucketed `CanonicalHal` against a
...
- `src/stl/learning/` for the RL wrapper contract.
- `src/stl/config/` for a versioned baseline configuration.
...
- `src/stl/learning/targets.py`
- a focused replay/schema module under `src/stl/learning/`
- `src/stl/learning/model.py` and `src/stl/learning/train.py`
- learning tests
...
- `src/stl/learning/model.py`
- `src/stl/learning/train.py`
...
- [x] Run the selected seed-4 checkpoint on the ruler. Best grouped-validation
  ...
  `src/stl/outputs/regen2rl/gen0_checkpoint_v3_full_s4/external_ruler_report.json`.
...
- replace or explicitly rename `src/stl/learning/support/self_play.py`
...
- [ ] **P5.3 No legacy path.** Core self-play may not import `CanonicalHal`,
  `src/stl/play/action_model.py`, `src/stl/play/search.py`, scripted opponents, reward
...
- replay/schema modules under `src/stl/learning/`
- learner orchestration under `src/stl/commands/`
- `src/stl/learning/support/reanalysis.py`
...
- `src/stl/learning/gates.py` and support modules
- `src/stl/commands/compare_*`, `strength_gate.py`, and `promote.py`
...
R1 is a follow-on plan, not permission to modify `src/crates/` during P0-P8.

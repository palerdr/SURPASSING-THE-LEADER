...
def _source_tree_digest(repo_root: Path) -> str:
    paths = [
        *repo_root.glob("src/stl/**/*.py"),
        *repo_root.glob("src/stl/config/**/*.yaml"),
            ...
            for anchor in anchors:
                ...
                targets.append(
                    ValueTarget(
                        ...
                        source_artifact=(
                            f"src/stl/learning/tactical_anchors.py::{anchor.name}"
                        ),
    ...
    for stratum, entries in strata.items():
        ...
        for index, (name, scenario) in enumerate(ordered):
            ...
            target = ValueTarget(
                ...
                target_kind="tablebase_value",
                state_origin=StateOrigin.TACTICAL_TABLEBASE.value,
                source_artifact=f"src/stl/solver/tablebase.py::{name}",
                source_artifact_digest=source_digest,
        ...
        if args.profile == "scaled_v4":
            seal_payload = write_holdout_seal(
                getattr(
                    args,
                    "holdout_seal_out",
                    "src/stl/outputs/regen2rl/gen0_v4_holdout.seal.json",
                ),
            ...
            _log(
                "sealed holdout: "
                f"{getattr(args, 'holdout_seal_out', 'src/stl/outputs/regen2rl/gen0_v4_holdout.seal.json')} "
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--train-out", default="src/stl/outputs/regen2rl/gen0_train_v3.npz")
    parser.add_argument("--dev-out", default="src/stl/outputs/regen2rl/gen0_development_v3.npz")
    parser.add_argument(
        "--ruler-out", default="src/stl/outputs/regen2rl/gen0_external_ruler_v3.npz"
    )
    parser.add_argument("--work-dir", default="src/stl/outputs/regen2rl/gen0_work_v3")
    parser.add_argument("--tier-a-dir", default="src/stl/checkpoints/tablebase/tier_a")
    ...
    parser.add_argument(
        "--holdout-seal-out",
        default="src/stl/outputs/regen2rl/gen0_v4_holdout.seal.json",
    )
    parser.add_argument(
        "--profile",

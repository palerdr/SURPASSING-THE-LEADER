...
let legal_drop_max game player =
  let normal = (Game.config game).Dth_engine.Config.turn.duration_normal in
  if player = Dth_engine.Domain.Baku && Game.is_leap_second_turn game then
    Game.get_turn_duration game
  else normal

let legal_check_max game =
  (Game.config game).Dth_engine.Config.turn.duration_normal
...
let play_match term mode =
  ...
  while not (Game.game_over !game) do
    ...
    let drop_max = legal_drop_max !game dropper.Dth_engine.Player.id in
    let check_max = legal_check_max !game in

...
(* AF: A value [g : t] represents a complete game state of Drop The
   ...
   history of all resolved half-rounds in reverse chronological order,
   the player chosen as first dropper at game creation, Baku's and Hal's
   per-player state (cylinder,
   deaths, ttd, alive flag), winner and loser if the game is decided, whether
   the game is over, a deterministic RNG state from which all future survival
   rolls draw, the immutable Config.t supplying every tunable parameter).
   ...
   to 0). - game_over <=> (winner <>
   None && loser <> None). - winner = Some p iff loser = Some (the other
   player). - If a player_state has status = Dead (permanent), then game_over =
   true and that player is the loser. *)
type t = {
  config : Config.t;
  game_clock : int;
  round_number : int;
...
let handle_death (game : t) (checker : Player.player_state)
    ~(st_in_vial : int) ~(death_duration : int)
    (survived_outcome : bool option) : Player.player_state * bool * float =
...
let resolve_check (game : t) (checker : Player.player_state) (drop_time : int)
  ...
  if check_time >= drop_time then
    ...
    | Overflow ->
        ...
        in
        let game' : t =
          if checker.id = Domain.Baku then
            { game with baku_state = new_player }

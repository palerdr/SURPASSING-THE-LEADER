(* Drop The Handkerchief — unified revival model.
   ...
   CPR count and identity-specific physicality are not state inputs. The
   referee burden is represented by a TTD-derived factor, so equivalent physical
   states receive equivalent odds regardless of player identity or history
   encoding. *)
...
let compute_revival_prob (config : Config.t) ~(st_in_vial : int)
  ...
  if st_in_vial < 0 then invalid_arg "st_in_vial must be nonnegative"
    ...
    then 0.0
      ...
      let st_factor =
        1.0
        -. (float_of_int st_in_vial /. float_of_int survivable_st_span)
      in
      let normalized_ttd =
...
let attempt_revival (config : Config.t) ~(st_in_vial : int)
    ~(ttd_accrued : int) (rng : rng) : bool =
  let revival_prob =
    compute_revival_prob config ~st_in_vial ~ttd_accrued
  in

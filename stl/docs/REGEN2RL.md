...
- [x] **P3 MCTS improvement operator.** Canonical linearly weighted mean-Q
  ...
  `stl/outputs/regen2rl/p3_mcts_conformance.json`, canonical report-payload SHA-256
...
1. The audit found that the former `stl/config/command/self_play.yaml` dispatched
...
- `stl/config/` for a versioned baseline configuration.
...
- [x] Run the selected seed-4 checkpoint on the ruler. Best grouped-validation
  ...
  `stl/outputs/regen2rl/gen0_checkpoint_v3_full_s4/external_ruler_report.json`.

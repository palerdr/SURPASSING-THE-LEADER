...
from stl.learning.evaluate import evaluate, get_nn_model
...
def _search_depth1_batched(
    ...
    from stl.learning.model import extract_features, DEVICE
        ...
        for j, b_bucket in enumerate(pruned_baku):
            ...
            if not causes_death:
                ...
                sp = game.referee.compute_survival_probability(chk, death_duration=min(
                    chk.cylinder + ((check_time - drop_time + 1) if success else FAILED_CHECK_PENALTY),
                    CYLINDER_MAX,
                ))
...
def _simulate_and_recurse(
    ...
    sp = game.referee.compute_survival_probability(checker, death_duration=min(
        checker.cylinder + ((check_time - drop_time + 1) if success else FAILED_CHECK_PENALTY),
        CYLINDER_MAX,
    ))

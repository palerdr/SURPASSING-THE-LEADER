...
SOURCE_SELF_PLAY_MCTS = "self_play_mcts"
SOURCE_POLICY_GUARD = "policy_guard"
SOURCE_OPPONENT_TRACE_GUARD = "opponent_trace_guard"
...
class LabelResult:
    ...
    exact_result: ExactSolveResult | None = None
...
def _legal_policy_vectors(
    ...
    drop_seconds = tuple(
        legal_seconds_for_current_role(game, dropper.name, "dropper", config)
    )
    check_seconds = tuple(
        legal_seconds_for_current_role(game, checker.name, "checker", config)
    )
...
def game_from_public_state(state: ExactPublicState | dict) -> Game:
    ...
    first_dropper = (
        by_name.get(s["first_dropper_name"]) if s["first_dropper_name"] else None
    )
...
def _is_lsr_pressure(game: Game) -> bool:
    ...
    hal_cyl = float(
        ...
        if game.player1.name.lower() == "hal"
        else game.player2.cylinder
    )
    baku_cyl = float(
        game.player1.cylinder
        if game.player1.name.lower() == "baku"
        else game.player2.cylinder
    )
...
def label_state(
    ...
    if tval is not None:
        drop_dist, check_dist, drop_mask, check_mask = _legal_policy_vectors(
            game, config
        )
        return LabelResult(
            value=tval,
    ...
    if state in pinned_table:
        drop_dist, check_dist, drop_mask, check_mask = _inactive_policy_vectors(
            game, config
        )
        return LabelResult(
...
def _death_grid_pairs(
    *,
    ...
    return tuple(
        (baku_deaths, hal_deaths)
        for baku_deaths in baku_axis
        for hal_deaths in hal_axis
    )
...
def generate_targets(
    *,
    baku_cylinder_grid: tuple[float, ...] = (
        0.0,
        60.0,
        120.0,
        180.0,
...
def generate_mcts_bootstrap_targets(
    ...
    *,
    ...
    evaluator = ValueNetEvaluator(
        model_fn=predict_fn, default_value_horizon=2
    )
    death_pairs = _death_grid_pairs(
        deaths_grid=deaths_grid,
        baku_deaths_grid=baku_deaths_grid,
...
def to_training_record_v3(
    ...
    *,
    ...
    return TrainingRecordV3(
        ...
        or f"anchor-{exact_state_hash(target.exact_state)}",
...
def save_targets(targets: list[ValueTarget], path: str | Path) -> None:
    ...
    if targets:
        ...
        dropper_masks = np.stack([t.dropper_legal_mask for t in targets]).astype(
            np.float32
        )
        checker_masks = np.stack([t.checker_legal_mask for t in targets]).astype(
            np.float32
        )
    else:
...
def load_targets_as_records(path: str | Path) -> list[ValueTarget]:
    ...
    unresolved = (
        data["unresolved_probabilities"].astype(np.float32)
        if "unresolved_probabilities" in data
        else np.zeros(n, dtype=np.float32)
    )
    exact_states_json = (
        np.asarray(data["exact_states_json"]).astype(str)
        if "exact_states_json" in data
        else np.full(n, "", dtype=np.str_)
    )
    target_kinds = (
        np.asarray(data["target_kinds"]).astype(str)
        if "target_kinds" in data
        else np.asarray(
            [_target_kind_for_source(str(source)) for source in sources], dtype=np.str_
        )
    )
    for name, arr in (
        ("dropper_dists", dropper_dists),

...
`dth/solver.py` is the executable authority for this abstraction. This
...
Hydra loads `dth/config/dataset.yaml`; `horizon`, `output`, `root_states`, and
`progress_every` are command-line overridable. `dth/solver.py` performs no

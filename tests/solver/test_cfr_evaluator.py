...
def test_value_net_evaluator_returns_model_fn_value():
    evaluator = ValueNetEvaluator(model_fn=lambda game, *, horizon: 0.42)
    assert _scalar(evaluator(_make_fresh_game(), value_horizon=2)) == 0.42
...
def test_value_net_evaluator_passes_game_to_model_fn():
    ...
    def model_fn(game: Game, *, horizon: int) -> float:
        assert horizon == 2
    ...
    assert _scalar(evaluator(g, value_horizon=2)) == 173.0


def test_value_net_evaluator_satisfies_leaf_evaluator_protocol_structurally():
    def call_with_protocol(evaluator: LeafEvaluator, game: Game):
        return evaluator(game, value_horizon=2)

    evaluator = ValueNetEvaluator(model_fn=lambda game, *, horizon: -0.3)
...
def test_value_net_evaluator_composes_as_tablebase_fallback():
    ...
    fallback = ValueNetEvaluator(model_fn=lambda game, *, horizon: 0.25)
    evaluator = TablebaseEvaluator(fallback=fallback)

    pinned = get_scenario("forced_baku_overflow_death")
    assert _scalar(evaluator(pinned.game)) == 1.0           # tablebase hit
    assert _scalar(evaluator(_make_fresh_game(), value_horizon=2)) == 0.25

...
import stl.learning.targets as vt
...
def _tablebase_game():
    ...
    return _build_game(baku_cylinder=300.0)


def _lsr_significant_non_tablebase_game():
    """clock=2000 half=1, Hal drops, Baku checks at cyl=300, cprs=1 (off-pin).

    - rounds_until_leap_window=6 (so the near-leap gate does NOT fire here),
      but current_checker_fail_would_activate_lsr=True at half=1 with these
      timings, which is enough to flag this state as LSR-significant.
    - baku_cylinder=300 forces every cell of the half-1 matrix to terminate,
      so horizon=3 collapses to a one-step LP that resolves in <100ms.
    - referee_cprs=1 differs from the pinned forced_baku_overflow_death entry
...
def _lsr_pressure_non_significant_game():
    """clock=720 half=2 hal=300 cprs=1: pressure, not near leap/significant.

    - rounds_until_leap_window=11, cflsr=False, so NOT LSR-significant.
    - hal_cylinder=300 forces every checker cell to terminate, so horizon=2
...
def _tiny_grids() -> dict:
    """Minimal grid: one cyl=0 (excluded) + one cyl=300 (tablebase)."""
    return dict(
...
def _h2_only_grid() -> dict:
    ...
    clock=720 half=2 hal=300 cprs=1: pressure, not significant, and
    forced-terminal so horizon=2 resolves quickly.
    """
    return dict(
        baku_cylinder_grid=(0.0,),
        hal_cylinder_grid=(300.0,),
        clock_grid=(720.0,),
        half_grid=(2,),
        deaths_grid=(0,),
        cpr_grid=(1,),
...
def _h3_only_grid() -> dict:
    ...
    clock=2000 half=1 baku=300 cprs=1: cflsr=True (LSR-significant),
    forced-terminal so horizon=3 resolves quickly.
    """
    return dict(
        baku_cylinder_grid=(300.0,),
        hal_cylinder_grid=(0.0,),
        clock_grid=(2000.0,),
        half_grid=(1,),
        deaths_grid=(0,),
...
def test_generate_targets_skips_excluded_states():
    """Default state alone yields zero targets; cyl=300 alone yields one."""
...
def test_value_target_records_unresolved_probability_for_horizon_2(monkeypatch):
    ...
    assert result.dropper_dist[1] == pytest.approx(0.75)
    assert result.checker_dist[60] == pytest.approx(0.6)

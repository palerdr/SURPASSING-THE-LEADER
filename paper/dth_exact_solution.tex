...
\]
Setting every difference to zero gives $\sigma_{c,d}$ from the later
entries. Start at $\sigma_{c,60}=1$ and step down $59$ times: this is the
recurrence with $r_k=\sigma_{c,61-k}$. By Lemma~\ref{lem:persym},
$(\sigma_d^{\mathsf T}M)_c=(M\sigma_c)_{61-c}$ is the same constant.
\end{proof}

Negative entries of $r$ are clipped and the pair is normalized before the
certificate is measured. The clip cannot invalidate the bounds, which hold for
any mixed pair, and Lemma~\ref{lem:persym} holds for any reversed pair, so one
product $M\sigma_c$ still yields both $U$ and $L$. Each class then goes
...
\item \textbf{Equalizer.} Proposition~\ref{prop:equalizer}: $1{,}770$
  ...
  minimum are $U$ and $L$ (Lemma~\ref{lem:persym}).

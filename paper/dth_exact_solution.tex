...
\title{Exact State Compression and Certified Backward Induction\\
for the Drop-the-Handkerchief Timing Model}
...
against its full $60\times60$ stage matrix. The same local certificates bound
the exploitability of the resulting full-game policy. At the initial state,
$V=0.08985\pm0.00061$, so the first Dropper wins with probability
$0.54493\pm0.00031$ under optimal play; the selected policy has full-game
payoff gap at most $1.201\times10^{-3}$. The largest continuous run covered
...
$v^{\mathrm{succ}}_1,\dots,v^{\mathrm{succ}}_{61-d}$ together with
$v^{\mathrm{fail}}$ when $d>1$, and column $c$ contains the prefix through
$v^{\mathrm{succ}}_c$ together with $v^{\mathrm{fail}}$ when $c<60$. Hence
with prefix scans
$m_j=\min_{i\le j}v^{\mathrm{succ}}_i$ and
$M_j=\max_{i\le j}v^{\mathrm{succ}}_i$,
\begin{align*}
\max_d\min_c M(x)_{d,c}
&=\max_d
\begin{cases}
m_{60}, & d=1,\\
\min(m_{61-d},v^{\mathrm{fail}}), & d>1,
\end{cases}\\[3pt]
\min_c\max_d M(x)_{d,c}
&=\min_c
\begin{cases}
\max(M_{c},v^{\mathrm{fail}}), & c<60,\\
M_{60}, & c=60,
\end{cases}
\end{align*}
...
success values, never an arbitrary subset. Two linear scans therefore
...
$\ell=c-d+1$ is constant on diagonals, while every failed check has the same
value $v^{\mathrm{fail}}$. Row minima and column maxima reduce to prefix scans
...
profile is failure-fatal, $p\equiv0$ and $v^{\mathrm{fail}}=+1$ with no child;
...
\subsection{From local certificates to a policy}

Choose at every class the mixed pair whose accepted midpoint is
$\widehat V(x)$. These choices form a behavioral policy
$\widehat\pi=(\widehat\pi_d,\widehat\pi_c)$ for the whole game. Let
$J_x(\pi_d,\pi_c)$ be the current Dropper's expected terminal payoff under
two full behavioral strategies. The subscripts name the players' roles at
$x$; each strategy also specifies play after later role swaps. Define
\[
\underline J_{\widehat\pi}(x)
=\inf_{\pi_c}J_x(\widehat\pi_d,\pi_c),\qquad
\overline J_{\widehat\pi}(x)
=\sup_{\pi_d}J_x(\pi_d,\widehat\pi_c).
\]
Their difference
$\Gamma_{\widehat\pi}(x)=\overline J_{\widehat\pi}(x)
-\underline J_{\widehat\pi}(x)$ is the full-game policy gap: deviations may
change every later action, not only the next one.

\begin{proposition}[Propagated policy certificate]\label{prop:policy}
Let $g(x)$ be the accepted local gap at class $x$. Under the certificate
convention of Definition~\ref{def:certificate}, the selected policy satisfies
\[
\Gamma_{\widehat\pi}(x)
\le
\max_{\mathcal P:x\leadsto\mathrm{terminal}}
\sum_{z\in\mathcal P}g(z)
\le \delta(x)\tol
\le(1201-\Phi(x))\tol.
\]
At the initial state, the payoff gap is at most
$1.201\times10^{-3}$. Thus no unilateral deviation can improve a player's
win probability by more than $6.005\times10^{-4}$.
\end{proposition}

\begin{proof}
Let $B(x)$ be half the largest path sum in the first bound. We prove backward
that
\[
\underline J_{\widehat\pi}(x)\ge\widehat V(x)-B(x),
\qquad
\overline J_{\widehat\pi}(x)\le\widehat V(x)+B(x).
\]
The claim is exact at a terminal. At a live state, the role swap only negates
the child payoff, and revival only averages it. Neither operation enlarges
the largest child bound. The selected Dropper strategy therefore guarantees
$L_x-\max_y B(y)$, while the selected Checker strategy caps the payoff at
$U_x+\max_y B(y)$. Since
$\widehat V(x)=(L_x+U_x)/2$ and $U_x-L_x\le g(x)$, these imply the two
displayed bounds. Subtracting them gives the path sum. Finally, a path contains at most
$\delta(x)\le1201-\Phi(x)$ live states.
\end{proof}
...
\end{axis}
\end{tikzpicture}
\caption{The root component of the selected behavioral policy. Its local gap
against the assembled root matrix is $3.2\times10^{-15}$; Proposition
\ref{prop:policy} bounds deviations through the rest of the game. Both
strategies have full support.}
\label{fig:root}
\end{figure}

As a check against a silent implementation error, the Python and Rust sweeps
agreed byte-for-byte wherever both computed the same region, and re-solving
$4{,}792$ sampled Bellman equations gave a worst deviation of
$4.84\times10^{-7}$. These checks support the implementation; the theorem's
error bound comes from Proposition~\ref{prop:accumulated}.
...
\]
The selected behavioral policy also satisfies
$\Gamma_{\widehat\pi}(0,0,0,0)\le1.201\times10^{-3}$.
\end{theorem}
...
\item \textbf{Values stored, policies reconstructed.} The table stores values
  and routing, not every local strategy. A policy component is re-derived
  from stored child values when needed. Proposition~\ref{prop:policy} applies
  to a full selection of accepted local pairs; Figure~\ref{fig:root} shows its
  root component.
\end{itemize}
...
\item builds $v^{\mathrm{succ}}_1,\dots,v^{\mathrm{succ}}_{60}$ and
  $v^{\mathrm{fail}}$ from already solved children;
...
Warm supports come from the two lag-one neighbours $(T_1p_c,p_d)$ and
$(p_c,T_1p_d)$. Here $T_1$ adds one second of ST and applies the quotient.
Actions with mass at most $10^{-9}$ are removed; if more than twelve remain,
the twelve largest masses are kept and sorted. If the two support sizes
differ, let $k=\min(|D|,|C|)$ and use the first $k$ actions in ascending
order. The equalizer then solves
\[
...
\end{aligned}
\]
Solver retries change the numerical path, not the acceptance gap.
...
\texttt{src/dth/config/complete\_full\_v1.yaml}). The stored float64 root
midpoint is \texttt{0.0898500728095116}.

...
\title{An Exact Solution of the Drop the Handkerchief Game}
\author{James Cenawood\thanks{Code and reproduction instructions:
\url{https://github.com/palerdr/SURPASSING-THE-LEADER}}}
\date{August 25, 2026}
...
$60\times60$ stage matrix. A round pays out on the gap between the two chosen
seconds and on nothing else. The marginal value of a one-second delay is
therefore the same short expression at every second. One 59-step recurrence
solves it and gives both equilibrium strategies. One source file builds
the whole table in $49$ seconds on a fifteen-core laptop. At the root state
...
Two numbers describe a player. The ST is the time in the cylinder that a
failed check would inject, before the fixed sixty-second penalty. The TTD is
the total damage from the doses that the player already took. The cylinder
holds 300 seconds, so a large ST or a large TTD makes the next failed check
fatal. The ST of the player who checks grows round by round, until a failed
check empties the cylinder into them.

The tension of one round is short to state. The Checker wants to check on the
same second as the drop, because that check succeeds and costs only one
second of ST. A check one second early instead fails and brings the needle.
The Checker therefore trades a small lag against the risk of failure, and the
Dropper hides the drop second.
...
When the transitions form a DAG, the later values substitute backward. The
value $V(x)$ is therefore the payoff that optimal play from both sides gives.
Since $V=P(\mathrm{win})-P(\mathrm{loss})$, $P(\mathrm{win})=(1+V)/2$.
...
The two symbols carry the whole round. $S_\ell$ is what the Dropper gets when
the check succeeds at lag $\ell$, and $F$ is what the Dropper gets when the
check fails. A pair of seconds only selects one of these $61$ numbers. The
implementation reads $61$ values out of $V$ and never builds the $3{,}600$
cells.
...
min and max select one of their arguments and do not round.
\end{lemma}
...
progress score that orders the states. Backward induction needs each child
before its parents, and the potential supplies that order without a search.
...
No transition stays inside a $\Phi$-layer. Thus a sweep down the layers
solves every child before its parents. With the one-player bucket
...
Proposition~\ref{prop:midpoint} is the arbiter for the whole build. $L$ is the
payoff that the Dropper's mixture guarantees, and $U$ is the payoff that the
Checker's mixture allows. Weak duality puts the true value between them for
every mixed pair. The bounds therefore do not depend on the method that made
the pair. A faulty candidate does not give a wrong value: it gives a wide gap,
and the class moves to the next rung. The rungs below only propose a pair;
this definition decides.

\begin{lemma}[Mirror symmetry]\label{lem:persym}
The stage matrix is persymmetric, $M_{d,c}=M_{61-c,\,61-d}$, because the map
$(d,c)\mapsto(61-c,\,61-d)$ preserves both the condition $c\ge d$ and the
lag $c-d+1$ in \eqref{eq:toeplitz}. Hence for every $\sigma\in\Delta_{60}$
and its reversal $\bar\sigma_i=\sigma_{61-i}$,
$(\sigma^{\mathsf T}M)_c=(M\bar\sigma)_{61-c}$ for $c=1,\dots,60$.
\end{lemma}

The mirror is a statement about the clock, not about the players. A round
pays out on the gap between the two seconds, and the two roles read that gap
from opposite ends. One product $M\sigma_c$ therefore gives $U$ and $L$
together, for any $\sigma_c$ paired with its reversal.

\subsection{The marginal value of a one-second delay}\label{sec:marginal}

A pure saddle point needs one cell that is best for both players at once, and
Lemma~\ref{lem:toeplitz} finds that cell in constant time. When no such cell
exists, both players must mix. The reason is an expected-value argument, and
it does not need the matrix.

Let the Checker play the mixture $\sigma_c$, and write $q_j=\sigma_{c,j}$.
Definitions~\ref{def:actions} and~\ref{def:failure} give the Dropper's
expected value at drop second $d$ at once. A check before the drop fails and
pays $F$. A check at second $c\ge d$ succeeds at lag $c-d+1$ and pays
$S_{c-d+1}$. Therefore
\begin{equation}\label{eq:ev}
\EV(d)\;=\;F\sum_{c<d}q_c\;+\;\sum_{c\ge d}q_c\,S_{c-d+1}
\;=\;(M\sigma_c)_d .
\end{equation}

The Dropper reads $\EV$ and moves all mass to its largest entry. The
Checker's mixture therefore holds the Dropper to a mixture only when every
drop second that the Dropper uses has the same expected value. This rung
takes the full-support case, in which all $60$ drop seconds are equal. That
gives $59$ equations, one for each step from $d$ to $d+1$, and the sum of the
mixture gives the sixtieth.

\begin{proposition}[The margin of a delay]\label{prop:marginal}
Let $\delta_0=S_1-F$ and $\Delta_j=S_{j+1}-S_j$. Then, for $d=1,\dots,59$,
\begin{equation}\label{eq:marginal}
\EV(d)-\EV(d+1)\;=\;
\underbrace{\delta_0\,q_d}_{\text{the boundary term}}
\;+\;\underbrace{\sum_{j=1}^{60-d}\Delta_j\,q_{d+j}}_{\text{the lag shift}} .
\end{equation}
\end{proposition}

\begin{proof}
Compare \eqref{eq:ev} at $d$ and at $d+1$. The mass $q_d$ crosses the
boundary. A check at second $d$ was the tightest success and paid $S_1$.
After the delay the same check comes before the drop and pays $F$. That swing
is $\delta_0$ for each unit of mass.

Every later check keeps its second and loses one second of lag. Its payoff
therefore moves from $S_{j+1}$ to $S_j$, a swing of $\Delta_j$ for each unit
of mass at $q_{d+j}$. Nothing else changes, because a check before second $d$
pays $F$ in both cases.
\end{proof}

Equation \eqref{eq:marginal} is the Toeplitz shape of \eqref{eq:toeplitz} in
expected-value language. A round pays out on the gap $c-d$ and on nothing
else. One second of delay therefore moves every lag by the same amount. The
coefficients $\delta_0,\Delta_1,\dots,\Delta_{59}$ are the same short stencil
at every $d$, and only the window into $q$ moves.

\begin{proposition}[One recurrence, both strategies]\label{prop:equalizer}
Let $\delta_0\ne0$ and define
\[
r_1=1,\qquad
r_k=-\frac{1}{\delta_0}\sum_{m=1}^{k-1}\Delta_m\,r_{k-m}\quad(k=2,\dots,60).
\]
Then $\sigma_c\propto(r_{60},\dots,r_1)$ makes $M\sigma_c$ constant, and
its reversal $\sigma_d\propto(r_1,\dots,r_{60})$ makes
$\sigma_d^{\mathsf T}M$ the same constant.
\end{proposition}

\begin{proof}
Set every margin \eqref{eq:marginal} to zero. The equation at $d$ then reads
\begin{equation}\label{eq:backsub}
q_d=-\frac{1}{\delta_0}\sum_{j=1}^{60-d}\Delta_j\,q_{d+j}.
\end{equation}
This equation reads no entry before index $d$, because a delay only re-prices
the checks at or after the drop. The system is therefore triangular. Start at
$q_{60}=1$ and take $59$ steps down: each equation gives exactly one new
entry. The $59$ equations are homogeneous in $60$ unknowns, so they fix only
the ratios, and the sum of the mixture fixes the scale.

Substitute $r_k=q_{61-k}$ to get the recurrence as stated. By
Lemma~\ref{lem:persym}, $(\sigma_d^{\mathsf T}M)_c=(M\sigma_c)_{61-c}$ is the
same constant.
\end{proof}

\begin{example}[Three seconds]\label{ex:toy}
Take $S=(5,2,9)$ and $F=4$, so $\delta_0=1$, $\Delta_1=-3$, and
$\Delta_2=7$. The margin at $d=2$ is $q_2-3q_3$ and the margin at $d=1$ is
$q_1-3q_2+7q_3$. Back-substitution \eqref{eq:backsub} from $q_3=1$ gives
$q_2=3$ and then $q_1=2$, so $\sigma_c=(\tfrac13,\tfrac12,\tfrac16)$. Read
the three expected values back from \eqref{eq:ev}:
\[
\EV(1)=\frac{10+6+9}{6},\qquad
\EV(2)=\frac{8+15+2}{6},\qquad
\EV(3)=\frac{8+12+5}{6},
\]
and all three are $\tfrac{25}{6}$. No drop second beats another, so
$U=\tfrac{25}{6}$. The reversal $\sigma_d\propto(1,3,2)$ gives the same
constant, so $L=U$ and the gap is zero.
\end{example}

The kernel clips the negative entries of $r$ to zero and normalizes the pair
before it measures the certificate. The clip cannot invalidate the bounds,
because Proposition~\ref{prop:midpoint} holds for any mixed pair.
Lemma~\ref{lem:persym} also holds for any reversed pair, so one product
$M\sigma_c$ still gives both $U$ and $L$. Each class then goes through the
same ladder:

\begin{enumerate}[leftmargin=*]
\item \textbf{Pure saddle.} Lemma~\ref{lem:toeplitz}. If the maximin and the
  minimax meet within $\tol$, a pure action pair is enough.
\item \textbf{Equalizer.} Propositions~\ref{prop:marginal}
  and~\ref{prop:equalizer}: $1{,}770$ multiply-adds for $r$ and $1{,}770$ for
  $M\sigma_c$. Its maximum and minimum are $U$ and $L$
  (Lemma~\ref{lem:persym}).
...
Rung 2 divides by $\delta_0$, so it has one singular direction. Rung 1 fences
that direction off before rung 2 runs.

\begin{proposition}[The singular direction is safe]\label{prop:singular}
The gap of rung 1 is at most $|\delta_0|$. Rung 1 therefore certifies every
class with $|\delta_0|\le\tol$, and rung 2 never divides by a quantity below
the tolerance.
\end{proposition}

\begin{proof}
Write $m=\min_\ell S_\ell$ and $\overline m=\max_\ell S_\ell$, so
$m\le S_1\le\overline m$. Take $\delta_0\ge0$ first, which puts $F=S_1-\delta_0$
below $S_1$. Lemma~\ref{lem:toeplitz} then gives the maximin
$\max(m,F)\ge S_1-\delta_0$ and the minimax $\min(\overline m,S_1)=S_1$.

Take $\delta_0<0$ next, which puts $F$ above $S_1$. The maximin is
$\max(m,S_1)=S_1$ and the minimax is
$\min(\overline m,F)\le S_1+|\delta_0|$. In both cases the two values differ
by at most $|\delta_0|$, and rung 1 runs first.
\end{proof}

Example~\ref{ex:toy} shows that the bound is tight. Rung 1 there brackets the
value in $[4,5]$, a gap of exactly $\delta_0=1$, while the true value is
$\tfrac{25}{6}$.
...
security values against every complete policy of the opponent, with
deviations after later role swaps included. Then
\[
...
$0.6\%$ to $2.2\%$ on the other seconds. Each mixture makes the other player
indifferent between all sixty seconds, which is what
Proposition~\ref{prop:marginal} asks of it. The Checker is the exact mirror
...
The implementation is one Python file of $481$ lines. It builds the
$17{,}011$ quotient profiles with their successful-check children, failure
child, and revival probability. It stores $V$ as a
$17{,}011\times17{,}012$ float64 array. The extra column holds $-1$, so a
terminal win reads as $+1$ after the role-swap negation, with no branch. It
lists each layer by Corollary~\ref{cor:layers}, in an order where consecutive
classes read the same row of $V$. One compiled kernel per layer runs in
parallel over the classes.
...
and the maximum of $M\sigma_c$ as it produces them. It writes the certified
midpoint and its rung. The build collects the classes that fail both rungs
and solves them by linear programming afterwards.
...
desktop in about $1.5$ hours. It used a warm-started support search, two
bordered $61\times61$ linear systems per class, and sessions with
checkpoints.
...
$334{,}588$ to the pure rung. It rejected the equalizer whenever an entry of
$r$ came out negative. The kernel here clips that entry, normalizes the pair,
and measures the gap anyway. Those classes are borderline ones with a gap at
the tolerance, and the clipped pair still certifies them. Both routes carry a
certificate, so the two builds agree on the values and differ only in the
route.

We compared six stored values with the earlier artifact. Five agree to
...
\item \textbf{Certified numerics.} Values are round-to-nearest float64
  ...
  enumeration.
\item \textbf{Shared code.} Definition~\ref{def:certificate} measures the gap
  against the matrix that the kernel itself assembles. A fault that corrupted
  the candidate and the check in the same way would pass. Two audits address
  that risk: the six reference values come from an earlier and independent
  implementation, and a separate scalar ladder re-solved the $1{,}200$
  strided classes.
...
\textbf{Reproducibility.} From \texttt{src/dth\_compact}, \texttt{uv run
main.py} builds the table, re-solves the $1{,}200$ audit classes, and prints
the six reference values in about one minute. \texttt{uv run pytest} runs
the eighteen structural and numerical checks.
